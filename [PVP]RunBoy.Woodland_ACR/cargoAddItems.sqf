...
// for each 35 boxes placed in editor with the name "hiddenBox_*" + index, add these items
for [{_y=1},{_y<=35},{_y=_y+1}] do
{	
	_random = floor random(100);

	_box = missionNamespace getVariable("hiddenBox_" + str _y);
	
	// In editor there is a "probability of spawning" variable set, so check if the box actually spawned.
	if ( !isNil "_box" ) then 
	{
